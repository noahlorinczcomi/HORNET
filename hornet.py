...
parser.add_argument('--isRawGTEx',action='store',type=str,default='no',help='(Required) If the eQTL association estimates are directly from the "eQTL Tissue-Specific All SNP Gene Associations" data sets from the GTEx Consortium, put True; else, put False.')
...
parser.add_argument('--cleanResults',action='store',type=str,default='yes',help='(Optional) Should the minimum interesting results be saved? Put "no" or "false" if you want the complete results, including causal estimates from other MR methods. The default is "yes", to only output the most important results')
### creating networks
